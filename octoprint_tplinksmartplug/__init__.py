		...
		if plugip != "":
			...
			if "ENE" in self.lookup(response, *["system","get_sysinfo","feature"]):
				...
				if self.lookup(check_emeter_data, *["emeter","get_realtime"]):
					...
					self.db = sqlite3.connect(self.db_path)
					cursor = self.db.cursor()
					cursor.execute('''INSERT INTO energy_data(ip, timestamp, current, power, total, voltage) VALUES(?,?,?,?,?,?)''', (plugip,datetime.today().isoformat(' '), emeter_data["get_realtime"]["current"], emeter_data["get_realtime"]["power"],emeter_data["get_realtime"]["total"],emeter_data["get_realtime"]["voltage"]))
					self.db.commit()
					self.db.close()
